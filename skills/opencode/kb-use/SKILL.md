---
name: kb-use
description: Retrieve what's already known from the local knowledge base (kb MCP) before searching code or docs. Tiered retrieval — KB first, then repo docs, then code. Use when answering questions about a project's architecture, decisions, gotchas, integrations, or before investigating a codebase.
---
...
- Only title, description and tags are indexed — a promising entry may hold much more in its `data` field, so fetch it in full (`get(id, include_data=True)`) before moving on.
